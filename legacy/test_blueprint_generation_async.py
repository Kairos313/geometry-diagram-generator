#!/usr/bin/env python3
"""
Test Blueprint Generation with Individual Prompts (Async Version)

This script tests the 4 specialized blueprint generation prompts by:
1. Sampling questions from each dimension type
2. Generating blueprints in PARALLEL using async/await
3. Validating JSON structure and key fields
4. Saving results for inspection

Usage:
    python3 test_blueprint_generation_async.py --sample-size 2
    python3 test_blueprint_generation_async.py --dimension coordinate_3d --sample-size 3
    python3 test_blueprint_generation_async.py --sample-size all  # Test all questions (slow!)
"""
...
from individual_prompts import (
...
async def generate_blueprint_async(api_key, question_text, dimension_type):
    # type: (str, str, str) -> Dict
    """Generate blueprint JSON using the appropriate specialized prompt (async wrapper).
    ...
    return await loop.run_in_executor(None, generate_blueprint_sync, api_key, question_text, dimension_type)


def generate_blueprint_sync(api_key, question_text, dimension_type):
    # type: (str, str, str) -> Dict
    """Synchronous blueprint generation (called from async wrapper)."""
    client = genai.Client(api_key=api_key)

    # Select the appropriate prompt based on dimension type
    prompt_map = {
    ...
    prompt_template = prompt_map.get(dimension_type, Question_to_Blueprint_Compact)
...
async def main_async():
    parser = argparse.ArgumentParser(description="Test blueprint generation with individual prompts (async)")
    parser.add_argument(
        '--sample-size',
        type=str,
        default='2',
    ...
    print(f"Running in PARALLEL mode (async)")
    print("=" * 80)
    print()

    # Prepare all tasks
    tasks = []
    task_metadata = []  # Store (dimension_type, question_index, question) for each task
    ...
    for dimension_type, questions in sorted(sampled.items()):
        ...
        for i, question in enumerate(questions):
            ...
            print(f"  Queuing: {q_id}")

            # Create async task
            task = generate_blueprint_async(api_key, q_text, dimension_type)
            tasks.append(task)
            task_metadata.append((dimension_type, i, question))
    ...
    print(f"All API calls completed in {total_duration:.2f}s")
    print("=" * 80)
    print()
    ...
    for (dimension_type, i, question), result in zip(task_metadata, results_raw):
        q_id = question['id']
        q_text = question['text']
        ...
        result_data = {
            ...
            "raw_output": result["raw_output"],  # Save raw output for debugging
        ...
        # Print status
        print(f"[{dimension_type}] {q_id}: {status}")
    ...
    print(f"\nTotal wall-clock time: {total_duration:.2f}s")
    print(f"Total API time: {sum(r['duration'] for r in results_raw):.2f}s")
    print(f"Speedup: {sum(r['duration'] for r in results_raw) / total_duration:.1f}x")
    ...
    summary = {
        ...
        "total_api_time": sum(r['duration'] for r in results_raw),
        "results": results
    }
