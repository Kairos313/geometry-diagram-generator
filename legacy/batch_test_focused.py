...
Uses a binary LLM classifier (Gemini 3 Flash) to detect 2d or 3d,
then routes to adaptive prompts that handle both traditional and coordinate
geometry (axes drawn when needed based on question content).
...
MAX_WORKERS = 5
CODEGEN_MODEL = "azure"    # "azure" | "openrouter"  (set by --codegen-model)
BLUEPRINT_MODEL = "gemini"  # "gemini" | "openrouter" (set by --blueprint-model)
...
PRICING = {
    ...
    "blueprint": {"input": 0.50, "output": 3.00},    # Gemini 3 Flash (updated if openrouter)
    "codegen": {"input": 0.28, "output": 0.42},      # DeepSeek-V3.2 Azure (same for openrouter)
}
...
def run_single_question(question):
    ...
    if not gemini_key:
        ...
        return result
    if CODEGEN_MODEL == "azure" and not deepseek_key:
        result.codegen.error = "DEEPSEEK_API_KEY not set (required for --codegen-model azure)"
        return result
    if (CODEGEN_MODEL == "openrouter" or BLUEPRINT_MODEL == "openrouter") and not openrouter_key:
        result.classify.error = "OPENROUTER_API_KEY not set (required for --codegen-model openrouter or --blueprint-model openrouter)"
        return result
    ...
    try:
        ...
        # Classifier outputs binary "2d"/"3d"; ground_truth may be "coordinate_2d"/"coordinate_3d"
        gt_base = result.ground_truth_dimension.replace("coordinate_", "")
        result.classifier_correct = (dimension_type == gt_base)
    ...
    try:
        bp_result = generate_blueprint(
            ...
            use_openrouter=(BLUEPRINT_MODEL == "openrouter"),
            openrouter_key=openrouter_key if BLUEPRINT_MODEL == "openrouter" else None,
        )
    ...
    # Classifier outputs binary "2d"/"3d"; use directly
    codegen_dim = dimension_type
    is_2d = dimension_type == "2d"
    ...
    for attempt in range(1, max_codegen_attempts + 1):
        try:
            code_result = generate_render_code(
                ...
                question_text=question["text"],
                compact=True,
                use_openrouter=(CODEGEN_MODEL == "openrouter"),
                openrouter_key=openrouter_key if CODEGEN_MODEL == "openrouter" else None,
            )
...
Examples:
  ...
  python3 batch_test_focused.py --workers 3                  # Limit concurrency
    ...
    parser.add_argument(
        ...
        default="azure",
        help="Code generation model: azure (default, DeepSeek-V3.2) or openrouter (DeepSeek-V3.2-Speciale with reasoning)"
    )
    parser.add_argument(
        "--blueprint-model",
        choices=["gemini", "openrouter"],
        default="gemini",
        help="Blueprint model: gemini (default, Gemini 3 Flash) or openrouter (DeepSeek-V3.2-Speciale with reasoning)"
    )
...
if __name__ == "__main__":
    ...
    # Update pricing if OpenRouter models are selected
    if BLUEPRINT_MODEL == "openrouter":
        PRICING["blueprint"] = {"input": 0.28, "output": 0.42}  # DeepSeek V3.2 pricing
    ...
    for d, c in sorted(dim_counts.items()):
        print("  {}: {}".format(d, c))
    blueprint_label = "DeepSeek-V3.2-Speciale (OpenRouter)" if BLUEPRINT_MODEL == "openrouter" else "Gemini 3 Flash"
    codegen_label = "DeepSeek-V3.2-Speciale (OpenRouter)" if CODEGEN_MODEL == "openrouter" else "DeepSeek-V3.2 (Azure)"
    print("Pipeline: Classify (Gemini) -> Blueprint ({}) -> CodeGen ({}) -> Execute".format(blueprint_label, codegen_label))
