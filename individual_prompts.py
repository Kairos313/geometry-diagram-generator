...
STAGE 1 - Question → Blueprint (4 prompts):
- Question_to_Blueprint_2D: Traditional 2D geometry (no axes)
- Question_to_Blueprint_3D: Traditional 3D geometry (no axes)
- Question_to_Blueprint_Coordinate_2D: 2D coordinate geometry (with axes, grid, equations)
- Question_to_Blueprint_Coordinate_3D: 3D coordinate geometry (with 3D axes, planes, spheres)

STAGE 2 - Blueprint → Code (4 prompts):
- Blueprint_to_Code_2D_DeepSeek: Traditional 2D → matplotlib
- Blueprint_to_Code_3D_DeepSeek: Traditional 3D → manim
- Blueprint_to_Code_Coordinate_2D: Coordinate 2D → matplotlib (with axes/grid)
- Blueprint_to_Code_Coordinate_3D: Coordinate 3D → manim (with 3D axes/planes/spheres)

Each prompt is optimized for its specific use case based on the dimension type
determined by the LLM classifier.

Helper functions:
- get_prompt_for_dimension(dimension_type) → Returns Stage 1 prompt
- get_code_prompt_for_dimension(dimension_type) → Returns Stage 2 prompt
"""
...
vector_label.rotate(PI/2, axis=RIGHT)
